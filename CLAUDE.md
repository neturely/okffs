...
- `create_issue` — when `OKFFS_PROJECT_AUTO_ADD=true`, adds the new issue to the board and optionally sets a `priority`. Two Priority shapes are supported: a **project-native single-select** field (set on the project item) and a GitHub **org-level Issue Field** (Phase 5.1, #91). When the project's Priority field reports no options — the signature of an org Issue Field — okffs resolves the option via `organization.issueFields` and sets it on the **issue** with `setIssueFieldValue`. Non-fatal, mirroring the `autoPR` block: any failure warns `[okffs]` and never blocks issue creation. `OKFFS_PROJECT_AUTO_ADD` is a fallback for boards without GitHub's native auto-add workflow.
...
- Token permission: fine-grained PAT → org "Projects: Read and write"; classic PAT → `project` scope. Missing permission surfaces a clear `[okffs]` 403 error. **Org-level Issue Fields (Priority) need a *different* permission** than Projects: a **classic PAT with `admin:org`** can read `organization.issueFields` and write via `setIssueFieldValue`; **fine-grained PATs currently return FORBIDDEN** for this preview API. When okffs's token can't reach it, the org-priority path degrades gracefully with an actionable `[okffs]` message (set the Priority manually, or use an `admin:org` classic token).
