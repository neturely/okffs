...
- Optional `.env` defaults: `OKFFS_DEFAULT_ASSIGNEES`, `OKFFS_DEFAULT_LABELS`, `OKFFS_DEFAULT_PRIORITY`, `OKFFS_PROMPT_METADATA`, `OKFFS_BASE_BRANCH`, `OKFFS_IDENTIFIER`, `OKFFS_UPDATE_DOCS`, `OKFFS_AUTO_PR`.
...
- `list_issues` — surfaces each issue's current board column (`project: <column>`) and its `priority:`, and orders the listing by Priority (Urgent → High → Medium → Low → unset). Priority comes from the project-native field, or the org Issue Field when `OKFFS_CLASSIC_PAT=true` (one extra batched query, gated on that flag). Non-fatal: the listing still renders if either Projects fetch fails.
