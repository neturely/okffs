...
- `merge_pull_request` is the **one okffs tool that merges** — every other tool only opens PRs and hands back. It autonomously merges a **green, review-resolved issue PR into the base branch** (e.g. `develop`) using `OKFFS_BASE_MERGE_METHOD` (default `squash`), then comments on the issue and — when the base isn't the repo default (so `Closes #N` won't auto-close) — closes the issue to complete the loop. **Opt-in and heavily gated** (#211): does nothing unless `OKFFS_AUTO_MERGE_BASE=true`; refuses if `OKFFS_PROTECTED_BRANCH` is unset; only ever merges the base tier and **never** `OKFFS_PROTECTED_BRANCH` (structurally — it only fetches the PR whose base is the base tier, which is enforced distinct from the protected branch); and independently verifies (not trusting branch-protection rules) that the PR is open/non-draft/conflict-free, that **all** commit statuses **and** check runs are green (no failing/pending), that GitHub doesn't report it `blocked`/`behind`, and that **every** review thread is resolved. Any unmet gate → an actionable `[okffs]` refusal, no merge. The `develop → main` promotion always stays a manual, user-driven merge/tag (`promote_branch` opens it; you merge).
