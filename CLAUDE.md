...
- `update_issue` mutates the **core fields** of an existing issue — `title`, `assignees`, `labels`, `milestone`, and/or `body` — via a single REST PATCH with the configured `GITHUB_TOKEN`. Fills the gap where `create_issue` only sets these at creation time, so renaming an issue or changing an assignee no longer drops to raw `gh issue edit` (wrong identity/scopes). Pass only the fields to change; omitted fields are untouched. **`labels`/`assignees` REPLACE the whole set** (GitHub PATCH semantics — not merged), and `[]` clears them. Requires at least one field. Board Priority/Effort stays with `set_issue_fields` and the Status column with `update_project_status` — those aren't issue fields (#203).
