...
- `update_guidance` (MCP prompt / slash command) reviews an issue's diff and makes minimal, substantive edits to CLAUDE.md when functionality/config/conventions changed (add a new tool to the list, document a new env var, etc.) — not a changelog append; skips when nothing substantive changed. `create_pull_request` nudges the agent to run it when `OKFFS_UPDATE_GUIDANCE=true`.
